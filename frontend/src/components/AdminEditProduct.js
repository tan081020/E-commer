...
const AdminEditProduct = ({
    ...
    const [data,setData] = useState({
      ...
      const handleSubmit = async (e) => {
        e.preventDefault()
        const responce = await fetch(SummaryApi.updata_product.url,{
          method: SummaryApi.updata_product.method,
          credentials:'include',
          headers:{
    ...
    <div className=' fixed w-full h-full bg-slate-200 bg-opacity-50 top-0 right-0 bottom-0 left-0 flex justify-center items-center'>
       <div className=' bg-white p-4 rounded w-full max-w-2xl h-full max-h-[80%] overflow-hidden'>
          ...
          <form className=" grid p-4 gap-2 overflow-y-scroll h-full pb-5" onSubmit={handleSubmit} >
            ...
            <button className=' px-3 py-2 bg-red-600 text-white mb-10 hover:bg-red-800'>chỉnh sửa sản phẩm</button>
          </form>
